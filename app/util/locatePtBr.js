const ptBr = {
  ...
  array: {
    ...
    includesSingle:
      "valor único de {{!label}} não corresponde a nenhuma tipo válido",
    ...
    includesRequiredUnknowns:
      "não contém {{unknownMisses}} valor(es) obrigatório(s)",
    includesRequiredKnowns: "não contém {{knownMisses}}",
    includesRequiredBoth:
      "não contém {{knownMisses}} e {{unknownMisses}} outros valor(es) obrigatório(s)",
    ...
    orderedLength:
      "na posição {{pos}} falha, pois a matriz deve conter até {{limit}} itens",
  ...
  object: {
    ...
    with:
      "!!{{mainWithLabel}} peer obrigatório não encontrado: {{peerWithLabel}}",
    without:
      "!!{{mainWithLabel}} peer conflitante ou proibido: {{peerWithLabel}}",
    ...
    and:
      "contém {{presentWithLabels}} sem seus peers obrigatórios {{missingWithLabels}}",
    nand:
      "!!{{mainWithLabel}} não deve existir juntamente com {{peersWithLabels}}",
    assert: "!!{{ref}} validação falha, pois {{ref}} falhou em {{message}}",
    rename: {
      multiple:
        "não pôde renomear {{from}} pois muitas renomeaçãoes estão desativadas e já havia sido renomeado para {{to}}",
      override:
        "não pôde renomear {{from}} pois sobrescrever está desabilitado e {{to}} já existe",
      regex: {
        multiple:
          "não pôde renomear {{from}} pois muitas renomeaçãoes estão desativadas e já havia sido renomeado para {{to}}",
        override:
  ...
  string: {
    ...
    uriCustomScheme:
      "dever ser uma uri válida que se assemelhe ao padrão {{scheme}}",
    isoDate: "deve ser uma data no padrão ISO 8601",
    ...
    ipVersion:
      "deve ser um IP válido nas versões {{version}} com o CIDR {{cidr}}"
  }
};
